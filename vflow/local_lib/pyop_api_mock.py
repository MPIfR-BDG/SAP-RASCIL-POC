...
class API:
  ...
  def tick(self,time,func):
    if self.stopTimer:
      ...
      func()
      thread = threading.Timer(float(time.replace('s','')), self.tick, [time,func])
      thread.start()
...
def test_queue_single_port():
  ...
  while api.test.hasnext("output"):
    d = ["a-%d" % i for i in xrange(counter*5, (counter+1)*5)]
...
def test_queue_multiple_ports():
  ...
  counter = 0
  while api.test.hasnext("output"):
    d = [("a-%d" % i, "b-%d" % i) for i in xrange(counter*5, (counter+1)*5)]
    assert api.test.read("output") == json.dumps(d)
    counter += 1
  assert counter == 2
  print("done")
